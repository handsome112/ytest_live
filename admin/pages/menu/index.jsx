...
const table_list_1 = require("@components/menu/table-list");
const common_1 = require("@components/common");
class Menus extends react_1.PureComponent {
    constructor() {
        super(...arguments);
        this.state = {
            pagination: {},
    ...
    async search(page = 1) {
        try {
            ...
            const resp = await menu_service_1.menuService.search(Object.assign(Object.assign({}, this.state.filter), { limit: this.state.limit, offset: (page - 1) * this.state.limit, sort: this.state.sort, sortBy: this.state.sortBy }));
            await this.setState({
                searching: false,
                list: resp.data.data,
    ...
    render() {
        ...
        <common_1.BreadcrumbComponent breadcrumbs={[{ title: 'Menus' }]}/>
        <page_1.default>
          <search_filter_1.SearchFilter onSubmit={this.handleFilter.bind(this)}/>
          <div style={{ marginBottom: '20px' }}></div>
          <table_list_1.TableListMenu dataSource={list} rowKey="_id" loading={searching} pagination={pagination} onChange={this.handleTableChange.bind(this)} deleteMenu={this.deleteMenu.bind(this)}/>
        </page_1.default>
      </react_1.Fragment>);
    }
}
