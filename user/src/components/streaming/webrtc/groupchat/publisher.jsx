...
class Publisher extends react_1.PureComponent {
    ...
    async handelWebRTCAdaptorCallback(info, obj) {
        const { webRTCAdaptor, settings, id, muteLocalMic, unmuteLocalMic } = this.props;
        const { conversationId, streamId } = this.state;
        if (info === constants_1.WEBRTC_ADAPTOR_INFORMATIONS.INITIALIZED) {
            ...
            const player = video_js_1.default(id, {
                liveui: true,
                controls: true,
                muted: true,
                controlBar: {
    ...
    render() {
        ...
        return (<video id={id} className={classnames_1.default('video-js', classNames)} hidden={!publish_started} autoPlay playsInline muted/>);
    }
}
